...
// File: contracts/nftweets/interface/INFTKEYMarketplaceV2.sol
...
// File: contracts/nftweets/NFTweets_Market.sol
...
/**
 * @title NFTKEY Marketplace contract V2 fork for internal NFTweets Marketplace
 * Note: Payment tokens usually is the Hector Network Stablecoin $TOR
 */

contract NFTweetsMarketplaceV1 is INFTKEYMarketplaceV2, Ownable, ReentrancyGuard {
    using Address for address;
    ...
    uint256 private _actionTimeOutRangeMin = 30 minutes; // 30 mins
    uint256 private _actionTimeOutRangeMax = 30 days; // One month - This can extend by owner is contract is working smoothly

    mapping(address => ERC721Market) private _erc721Market;
    mapping(address => bool) public _allowedCollection;
    ...
    /* This contract is needed only for onsite trades, for a full NFT Marketplace,
    *  check Atlantica by Hector Network
    */

    function allowNFTweetsContract(address _NFTweets, bool _allowed) public onlyOwner{
        _allowedCollection[_NFTweets] = _allowed;
    }
        ...
        nonReentrant
    {
        ...
        (uint256 _serviceFee) = _calculateFees( listing.value );

        Address.sendValue( payable(listing.seller), msg.value - _serviceFee );
    ...
    function acceptBidForToken(
        ...
        (uint256 _serviceFee) = _calculateFees( existingBid.value );

        _paymentToken.safeTransferFrom({
            from: existingBid.bidder,
            to: msg.sender,
            value: existingBid.value - _serviceFee
        });
        _paymentToken.safeTransferFrom({ from: existingBid.bidder, to: owner(),  value: _serviceFee });
     ...
     * Only allowed NFTweet Collection
    ...
    {
        if (_allowedCollection[erc721Address] == true) {
            if (_isTokenOwner(erc721Address, listing.tokenId, listing.seller) && (_isTokenApproved(erc721Address, listing.tokenId) ||
                _isAllTokenApproved(erc721Address, listing.seller)) && listing.value > 0 && listing.expireTimestamp > block.timestamp ) {
                isValid = true;
            }
        }        
    }
     ...
     */
    function _calculateFees(uint256 value)
        private
        view
        returns (uint256 _serviceFee)
    {
        uint256 _baseFractions = 1000 +  _serviceFeeFraction;
